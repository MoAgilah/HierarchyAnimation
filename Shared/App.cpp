...
void App::HandleUpdate(const double& deltaTime)
{
}
...
// Library Includes //
/////////////////////
#include "../Hierarchy/Hierarchy/imgui/imgui.h"
#include "../Hierarchy/Hierarchy/imgui//imgui_impl_dx11.h"
#include "../Hierarchy/Hierarchy/imgui//imgui_impl_win32.h"
...
int Run(App *pApp)
{
	...
	float secPerCount = 1.0f / oneFrame.QuadPart;
	oneFrame.QuadPart /= 60;	//change this to 2 for step
	...
	while (DoMessages())
	{
		...
		double deltaTime = (now.QuadPart - prevTime.QuadPart) * secPerCount;
		...
		char text[256];
		sprintf_s(text, "framerate = %f\n", pApp->GetFramerate());
		OutputDebugStringA(text);
		//set prev to current
		prevTime.QuadPart = now.QuadPart;
	}
